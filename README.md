...
```
docker run -d -e GO_SERVER_URL=... lyuma/gocd-agent-fedora-32:v20.6.0
```
...
docker run -d \
        ...
        lyuma/gocd-agent-fedora-32:v20.6.0
```
...
To configure SSL parameters, pass the parameters using the environment variable `AGENT_BOOTSTRAPPER_ARGS`. See [this documentation](https://docs.gocd.org/current/installation/ssl_tls/end_to_end_transport_security.html) for supported options.
...
```
docker run -v /path/to/godata:/godata -v /path/to/home-dir:/home/go lyuma/gocd-agent-fedora-32:v20.6.0
```
...
```
docker run -e GOCD_AGENT_JVM_OPTS="-Dfoo=bar" lyuma/gocd-agent-fedora-32:v20.6.0
```
